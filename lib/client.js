/**
 * Client half of dsh-sidebar-element-picker.
 *
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var PLUGIN_ID = 'dsh-sidebar-element-picker'
    /** The host route the panel and the composer talk to. */
    var INVOKE = '/dsh-sidebar-element-picker/invoke'
    /** The native tab type's implementation id (also the slot key). */
    var TYPE_ID = 'dsh-sidebar-element-picker:browser'
    ...
    var SHELL_PATH = '/__dsh_picker__/chrome.html'
    ...
    var STYLE_ID = 'dsh-sidebar-element-picker-style'

    var STYLE_CSS = [
      '.dsh-sep-button{display:inline-flex;align-items:center;justify-content:center;width:28px;height:28px;padding:0;border:none;border-radius:8px;background:transparent;color:var(--dsw-alias-label-secondary);cursor:pointer}',
      '.dsh-sep-button:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}',
      '.dsh-sep-button[data-active="true"]{color:var(--dsw-alias-state-business-primary)}',
      '.dsh-sep-pane{display:flex;flex-direction:column;width:100%;height:100%;min-height:0;background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-primary);font-family:var(--dsw-font-family)}',
      '.dsh-sep-strip{display:flex;align-items:center;gap:4px;flex:none;min-height:30px;padding:3px 6px;border-bottom:0.5px solid var(--dsw-alias-border-l1)}',
      '.dsh-sep-tabs{display:flex;align-items:center;gap:3px;flex:1 1 auto;min-width:0;overflow-x:auto;scrollbar-width:none}',
      '.dsh-sep-tabs::-webkit-scrollbar{display:none}',
      '.dsh-sep-tab{display:flex;align-items:center;gap:5px;flex:none;max-width:150px;height:24px;padding:0 4px 0 9px;border:0.5px solid var(--dsw-alias-border-l1);border-radius:8px;background:transparent;color:var(--dsw-alias-label-secondary);font:inherit;font-size:11px;cursor:pointer}',
      '.dsh-sep-tab:hover{background:var(--dsw-alias-interactive-bg-hover)}',
      '.dsh-sep-tab[data-active="true"]{background:var(--dsw-alias-bg-layer-2);border-color:var(--dsw-alias-border-l3);color:var(--dsw-alias-label-primary)}',
      '.dsh-sep-tabTitle{overflow:hidden;white-space:nowrap;text-overflow:ellipsis}',
      '.dsh-sep-tabClose{display:inline-flex;align-items:center;justify-content:center;width:15px;height:15px;flex:none;border-radius:4px;font-size:13px;line-height:1;opacity:.55;cursor:pointer}',
      '.dsh-sep-tabClose:hover{opacity:1;background:var(--dsw-alias-interactive-bg-hover)}',
      '.dsh-sep-newtab{display:inline-flex;align-items:center;justify-content:center;width:22px;height:22px;flex:none;border:0.5px solid var(--dsw-alias-border-l1);border-radius:7px;background:transparent;color:var(--dsw-alias-label-secondary);font:inherit;font-size:13px;cursor:pointer}',
      '.dsh-sep-newtab:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}',
      '.dsh-sep-stripTools{display:flex;align-items:center;gap:6px;flex:none;font-size:11px;color:var(--dsw-alias-label-secondary)}',
      '.dsh-sep-link{border:none;background:transparent;padding:0 2px;color:var(--dsw-alias-label-secondary);font:inherit;cursor:pointer}',
      '.dsh-sep-link:hover{color:var(--dsw-alias-state-error-primary)}',
      '.dsh-sep-frames{position:relative;flex:1 1 auto;min-height:0}',
      '.dsh-sep-frame{display:block;width:100%;height:100%;border:0;background:#fff;position:absolute;inset:0}',
      '.dsh-sep-frame[data-hidden="true"]{visibility:hidden;pointer-events:none;z-index:0}',
      '.dsh-sep-start{display:flex;flex-direction:column;gap:14px;height:100%;padding:16px 14px 20px;box-sizing:border-box;overflow-y:auto}',
      '.dsh-sep-hero{display:flex;align-items:center;gap:10px}',
      '.dsh-sep-heroGlyph{display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;flex:none;border-radius:11px;background:var(--dsw-alias-bg-layer-2);border:0.5px solid var(--dsw-alias-border-l3);color:var(--dsw-alias-state-business-primary)}',
      '.dsh-sep-heroText{min-width:0}',
      '.dsh-sep-heroTitle{margin:0;font-size:13px;font-weight:600;color:var(--dsw-alias-label-primary)}',
      '.dsh-sep-heroHint{margin:2px 0 0;font-size:11px;color:var(--dsw-alias-label-secondary)}',
      '.dsh-sep-composer{display:flex;gap:6px;width:100%}',
      '.dsh-sep-section{display:flex;flex-direction:column;gap:2px;min-width:0}',
      '.dsh-sep-sectionHead{display:flex;align-items:center;gap:6px;padding:0 4px 4px;font-size:11px;color:var(--dsw-alias-label-caption)}',
      '.dsh-sep-sectionAction{margin-left:auto;border:none;background:transparent;padding:0;color:var(--dsw-alias-label-caption);font:inherit;cursor:pointer}',
      '.dsh-sep-sectionAction:hover{color:var(--dsw-alias-state-error-primary)}',
      '.dsh-sep-sectionCount{margin-left:auto;font-variant-numeric:tabular-nums}',
      '.dsh-sep-list{display:flex;flex-direction:column;gap:2px}',
      '.dsh-sep-item{display:flex;align-items:center;gap:2px;border-radius:9px;border:0.5px solid transparent}',
      '.dsh-sep-item:hover{background:var(--dsw-alias-bg-layer-2);border-color:var(--dsw-alias-border-l1)}',
      '.dsh-sep-itemMain{display:flex;align-items:center;gap:9px;flex:1 1 auto;min-width:0;padding:6px;border:none;border-radius:9px;background:transparent;color:var(--dsw-alias-label-primary);font:inherit;text-align:left;cursor:pointer}',
      '.dsh-sep-itemBadge{display:inline-flex;align-items:center;justify-content:center;width:22px;height:22px;flex:none;border-radius:7px;background:var(--dsw-alias-bg-layer-2);border:0.5px solid var(--dsw-alias-border-l1);color:var(--dsw-alias-label-secondary);font-size:11px;font-weight:600}',
      '.dsh-sep-itemBadge.isKey{color:var(--dsw-alias-state-business-primary)}',
      '.dsh-sep-itemText{display:flex;flex-direction:column;min-width:0;gap:1px}',
      '.dsh-sep-itemTitle{font-size:12px;color:var(--dsw-alias-label-primary);overflow:hidden;white-space:nowrap;text-overflow:ellipsis}',
      '.dsh-sep-itemUrl{font-size:11px;color:var(--dsw-alias-label-caption);overflow:hidden;white-space:nowrap;text-overflow:ellipsis}',
      '.dsh-sep-itemX{flex:none;width:20px;height:20px;margin-right:4px;border:none;border-radius:6px;background:transparent;color:var(--dsw-alias-label-caption);font:inherit;font-size:13px;line-height:1;cursor:pointer;opacity:0}',
      '.dsh-sep-item:hover .dsh-sep-itemX{opacity:1}',
      '.dsh-sep-itemX:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-state-error-primary)}',
      '.dsh-sep-idleHint{margin:0;font-size:12px;line-height:18px;color:var(--dsw-alias-label-secondary)}',
      '.dsh-sep-form{display:flex;flex-direction:column;gap:8px;width:100%}',
      '.dsh-sep-row{display:flex;gap:6px}',
      '.dsh-sep-input{box-sizing:border-box;flex:1 1 auto;min-width:0;height:32px;padding:0 10px;border:0.5px solid var(--dsw-alias-border-l4);border-radius:8px;background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-primary);font-family:var(--dsw-font-family);font-size:13px;outline:none}',
      '.dsh-sep-input:focus{border-color:var(--dsw-alias-state-business-primary)}',
      '.dsh-sep-open{flex:none;height:32px;padding:0 14px;border-radius:8px;border:none;background:var(--dsw-alias-button-primary-fill);color:var(--dsw-alias-label-primary-foreground);font-family:inherit;font-size:13px;cursor:pointer}',
      '.dsh-sep-open:hover{background:var(--dsw-alias-button-primary-hover)}',
      '.dsh-sep-open:disabled{opacity:.4;cursor:not-allowed}',
      '.dsh-sep-error{margin:0;font-size:12px;color:var(--dsw-alias-state-error-primary)}',
      '.dsh-sep-notice{margin:0;font-size:11px;color:var(--dsw-alias-label-secondary)}',
      '.dsh-sep-savebar{display:flex;align-items:center;gap:9px;flex:none;padding:7px 9px;border-bottom:0.5px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-2)}',
      '.dsh-sep-saveGlyph{display:inline-flex;align-items:center;justify-content:center;width:22px;height:22px;flex:none;border-radius:7px;background:var(--dsw-alias-bg-layer-1);border:0.5px solid var(--dsw-alias-border-l1);color:var(--dsw-alias-state-business-primary)}',
      '.dsh-sep-saveText{display:flex;flex-direction:column;gap:1px;flex:1 1 auto;min-width:0}',
      '.dsh-sep-saveTitle{font-size:12px;color:var(--dsw-alias-label-primary);overflow:hidden;white-space:nowrap;text-overflow:ellipsis}',
      '.dsh-sep-saveSub{font-size:11px;color:var(--dsw-alias-label-caption);overflow:hidden;white-space:nowrap;text-overflow:ellipsis}',
      '.dsh-sep-saveGo,.dsh-sep-saveSkip{flex:none;height:24px;padding:0 10px;border-radius:7px;font:inherit;font-size:12px;cursor:pointer}',
      '.dsh-sep-saveGo{border:none;background:var(--dsw-alias-button-primary-fill);color:var(--dsw-alias-label-primary-foreground)}',
      '.dsh-sep-saveGo:hover{background:var(--dsw-alias-button-primary-hover)}',
      '.dsh-sep-saveSkip{border:0.5px solid var(--dsw-alias-border-l3);background:transparent;color:var(--dsw-alias-label-secondary)}',
      '.dsh-sep-saveSkip:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}',
      '.dsh-sep-flash{display:flex;align-items:center;gap:6px;flex:none;padding:5px 9px;border-bottom:0.5px solid var(--dsw-alias-border-l1);font-size:11px;color:var(--dsw-alias-label-secondary)}',
      '.dsh-sep-code{padding:1px 4px;border-radius:4px;background:var(--dsw-alias-bg-layer-2);font-family:ui-monospace,SFMono-Regular,Menlo,monospace}',
      '.dsh-sep-busy{flex:1 1 auto;display:flex;align-items:center;justify-content:center;color:var(--dsw-alias-label-secondary);font-size:12px}',
    ].join('')
    ...
    function BrowserPanel(props) {
      ...
      if (state === null) {
        return h('div', { className: 'dsh-sep-pane' }, h('div', { className: 'dsh-sep-busy' }, '正在恢复浏览器…'))
      }

      if (state.tabs.length === 0) {
        return h(
          'div',
          { className: 'dsh-sep-pane' },
          h(
            'div',
            { className: 'dsh-sep-start' },
            h(
              'div',
              { className: 'dsh-sep-hero' },
              h('span', { className: 'dsh-sep-heroGlyph', 'aria-hidden': 'true' }, h(CrosshairGlyph, { size: 18 })),
              h(
                'div',
                { className: 'dsh-sep-heroText' },
                h('p', { className: 'dsh-sep-heroTitle' }, busy ? '正在打开…' : '打开一个网址开始'),
                h('p', { className: 'dsh-sep-heroHint' }, '在下面的记录里点一个，或者直接输入。'),
              ),
            ),
            h(
              'form',
              { className: 'dsh-sep-composer', onSubmit: onSubmit },
              h('input', {
                className: 'dsh-sep-input',
                type: 'text',
                spellCheck: false,
                autoComplete: 'off',
              ...
              h('button', { className: 'dsh-sep-open', type: 'submit', disabled: busy }, busy ? '打开中…' : '打开'),
            ),
            error === '' ? null : h('p', { className: 'dsh-sep-error' }, error),
            notice === '' ? null : h('p', { className: 'dsh-sep-notice' }, notice),
            history.length === 0
              ? null
              : h(
                  'section',
                  { className: 'dsh-sep-section' },
                  h(
                    'div',
                    { className: 'dsh-sep-sectionHead' },
                    h('span', null, '最近访问'),
                    h('button', { className: 'dsh-sep-sectionAction', type: 'button', onClick: clearHistory }, '清空'),
                  ),
                  h(
                    'div',
                    { className: 'dsh-sep-list' },
                    ...history.map((entry) =>
                      h(
                        'div',
                        { className: 'dsh-sep-item', key: entry.url },
                        h(
                          'button',
                          {
                            className: 'dsh-sep-itemMain',
                            type: 'button',
                          ...
                          h('span', { className: 'dsh-sep-itemBadge', 'aria-hidden': 'true' }, h(GlobeGlyph, { size: 13 })),
                          h(
                            'span',
                            { className: 'dsh-sep-itemText' },
                            h(
                              'span',
                              { className: 'dsh-sep-itemTitle' },
                              entry.title === '' ? hostOf(originOf(entry.url)) : entry.title,
                            ),
                            h(
                              'span',
                              { className: 'dsh-sep-itemUrl' },
            ...
            logins.length === 0
              ...
              : h(
                  ...
                  h(
                    ...
                    h('span', { className: 'dsh-sep-sectionCount' }, String(logins.length)),
                  ),
                  h(
                    'div',
                    { className: 'dsh-sep-list' },
                    ...logins.map((entry) =>
                      h(
                        'div',
                        { className: 'dsh-sep-item', key: entry.origin },
                        h('span', { className: 'dsh-sep-itemBadge isKey', 'aria-hidden': 'true' }, h(KeyGlyph, { size: 12 })),
                        h(
                          'span',
                          { className: 'dsh-sep-itemText' },
                          h(
                          ...
                          h('span', { className: 'dsh-sep-itemUrl' }, hostOf(entry.origin)),
                        ),
                        h(
                          'button',
                          {
            ...
            history.length === 0 && logins.length === 0
              ? h(
                  'p',
                  { className: 'dsh-sep-idleHint' },
                  '打开后点页面工具栏的「拾取」（或按 ` 键），再点页面上的元素，即可把它作为 ',
                  h('code', { className: 'dsh-sep-code' }, '[标签][DOMn]'),
      ...
      return h(
        ...
        pendingLogin === null
          ...
          : h(
              'div',
              { className: 'dsh-sep-savebar' },
              h('span', { className: 'dsh-sep-saveGlyph', 'aria-hidden': 'true' }, h(KeyGlyph, { size: 14 })),
              h(
                'div',
                { className: 'dsh-sep-saveText' },
                h('span', { className: 'dsh-sep-saveTitle' }, `保存 ${hostOf(pendingLogin.origin)} 的账号密码？`),
                h(
                  'span',
                  { className: 'dsh-sep-saveSub' },
                  pendingLogin.username === '' ? '未填用户名' : `账号：${pendingLogin.username}`,
                ),
              ),
              h('button', { className: 'dsh-sep-saveGo', type: 'button', onClick: () => acceptLogin(pendingLogin) }, '保存'),
              h('button', { className: 'dsh-sep-saveSkip', type: 'button', onClick: () => setPendingLogin(null) }, '不用'),
            ),
        notice === ''
          ? null
          : h('div', { className: 'dsh-sep-flash' }, h('span', { className: 'dsh-sep-saveGlyph', 'aria-hidden': 'true' }, h(KeyGlyph, { size: 12 })), notice),
        h(
          'div',
          { className: 'dsh-sep-strip' },
          h(
            'div',
            { className: 'dsh-sep-tabs' },
            state.tabs.map((tab) =>
              h(
                'button',
                {
                ...
                h('span', { className: 'dsh-sep-tabTitle' }, labelOfTab(tab)),
                h(
                  'span',
                  {
                    className: 'dsh-sep-tabClose',
                    role: 'button',
                    'aria-label': '关闭标签页',
                    onClick: (event) => {
            ...
            h('button', { type: 'button', className: 'dsh-sep-newtab', title: '新建标签页', onClick: onNewTab }, '＋'),
          ),
          h(
            'div',
            { className: 'dsh-sep-stripTools' },
            h('span', null, count > 0 ? `已拾取 ${count}` : ''),
            count > 0
              ? h('button', { className: 'dsh-sep-link', type: 'button', onClick: onClear, title: '清空本对话的页面元素上下文' }, '清空')
              : null,
          ),
        ),
        h('div', { className: 'dsh-sep-frames' }, frames),
      )
    }
    ...
    function apply(ctx) {
      ...
      ctx.effect(
        ...
        'dsh-sidebar-element-picker: composer button',
      )
      ...
      ctx.effect(() => () => {
        ...
        service.right = undefined
      }, 'dsh-sidebar-element-picker: tab type')
    }
